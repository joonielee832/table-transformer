...
def objects_to_cells(bboxes, labels, scores, page_tokens, structure_class_names, structure_class_thresholds, structure_class_map):
    ...
    table_class_objects = [
        obj for obj in table_objects if obj['label'] == structure_class_map['table']]
    if len(table_class_objects) > 1:
        table_class_objects = sorted(
            table_class_objects, key=lambda x: x['score'], reverse=True)
    try:
        table_bbox = list(table_class_objects[0]['bbox'])
    except:
        table_bbox = (0, 0, 1000, 1000)

    tokens_in_table = [token for token in page_tokens if postprocess.iob(
        token['bbox'], table_bbox) >= 0.5]
    ...
    for cell1_num, cell1 in enumerate(cells):
            ...
            for current_row in range(max_row+1, num_rows):
                ...
                if not cell2['cell_text'] == '':
                    adj_bbox = [(max(cell1['bbox'][0], cell2['bbox'][0])+min(cell1['bbox'][2], cell2['bbox'][2]))/2-3,
                                cell1['bbox'][3],
                                (max(cell1['bbox'][0], cell2['bbox'][0]) +
                                 min(cell1['bbox'][2], cell2['bbox'][2]))/2+3,
                                cell2['bbox'][1]]
            ...
            for current_column in range(max_column+1, num_columns):
                ...
                if not cell2['cell_text'] == '':
                    adj_bbox = [cell1['bbox'][2],
                                (max(cell1['bbox'][1], cell2['bbox'][1]) +
                                 min(cell1['bbox'][3], cell2['bbox'][3]))/2-3,
                                cell2['bbox'][0],
        ...
        for adjacent_cell_num, props in adjacent_cell_props.items():
            ...
            adjacency_list.append(
                (cell1['cell_text'], cell2['cell_text'], props[0], props[1]))
...
def compute_metrics(mode, true_bboxes, true_labels, true_scores, true_cells,
    ...
    true_text_grid = np.array(grits.cells_to_grid(
        true_cells, key='cell_text'), dtype=object)
    ...
    pred_text_grid = np.array(grits.cells_to_grid(
        pred_cells, key='cell_text'), dtype=object)
    ...
    if mode == 'grits-all':
        ...
        true_cell_dilatedbbox_grid = np.array(
            grits.output_to_dilatedbbox_grid(true_bboxes, true_labels, true_scores))
        pred_cell_dilatedbbox_grid = np.array(
            grits.output_to_dilatedbbox_grid(pred_bboxes, pred_labels, pred_scores))
...
def compute_metrics_summary(sample_metrics, mode):
    ...
    simple_samples = [
        entry for entry in sample_metrics if entry['num_spanning_cells'] == 0]
...
def print_metrics_summary(metrics_summary, all=False):
    ...
    for table_type in ['simple', 'complex', 'all']:
        ...
        print("Results on {} tables ({} total):".format(
            table_type, metrics['num_tables']))
        ...
        if all:
            ...
            print_metrics_line("DAR_Con (original)",
                               metrics, 'dar_con_original')
...
def eval_tsr_sample(target, pred_logits, pred_bboxes, mode):
    ...
    true_bboxes = [elem.tolist()
                   for elem in rescale_bboxes(true_bboxes, true_img_size)]
    ...
    pred_bboxes = [elem.tolist()
                   for elem in rescale_bboxes(pred_bboxes, true_img_size)]
...
def visualize(args, target, result):
    ...
    bboxes_out_filepath = os.path.join(
        args.debug_save_dir, bboxes_out_filename)
    ...
    pred_bboxes = result['boxes'].detach().cpu()
    ...
    for bbox, label, score in zip(pred_bboxes, pred_labels, pred_scores):
        if ((args.data_type == 'structure' and not label > 5)
            or (args.data_type == 'detection' and not label > 0)
                and score > 0.5):
    ...
    if args.data_type == 'structure':
        img_words_filepath = os.path.join(
            args.table_words_dir, img_filename.replace(".jpg", "_words.json"))
        cells_out_filename = img_filename.replace(".jpg", "_cells.jpg")
        cells_out_filepath = os.path.join(
            args.debug_save_dir, cells_out_filename)
...
def filter_table(targets, results):
    ...
    for target, result in zip(targets, results):
        pred_boxes, pred_labels, pred_scores = result['boxes'], result['labels'], result['scores']

        #? Filter targets for table gt box (label 0)
        gt_idx = (target['labels']==0).nonzero()
        filtered_target = target.copy()
        filtered_target.update({
            'boxes': filtered_target['boxes'][gt_idx],
            'labels': filtered_target['labels'][gt_idx],
            'area': filtered_target['area'][gt_idx],
            'iscrowd': filtered_target['iscrowd'][gt_idx]
        })
        filtered_targets.append(filtered_target)

        #? Filter for class 0 and return box with highest score
        pred_indices = (pred_labels==0).nonzero()
        pred_labels = pred_labels[pred_indices]
        pred_scores = pred_scores[pred_indices]
        pred_boxes = pred_boxes[pred_indices]
        final_idx = torch.argmax(pred_scores)
        filtered_res = {
            'scores': pred_scores[final_idx],
            'labels': pred_labels[final_idx],
            'boxes': pred_boxes[final_idx]
        }
        filtered_results.append(filtered_res)
    
    return filtered_targets, filtered_results
...
def evaluate(args, model, criterion, postprocessors, data_loader, base_ds, device):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_every, header):
        ...
        #? Filter targets and predictions to output bmd table only
        if args.filter:
    ...
    print("Total time taken for {} samples: {}".format(
        len(base_ds), datetime.now() - st_time))
